...
const AddProducts = () => {
  ...
  const handleChange=(e)=> {
    setForm({...form,[e.target.name]:typeof e.target.value === 'number'? Number(e.target.value): e.target.value })
    setSent(true)
  }
  ...
  data.append("rate",rating.rate)
  data.append("count",rating.count)
 ...
 image.map((img) => fileToBase64(img))
    ...
    const updatedProducts = [...storedProducts,{ ...form ,   rating: {
        rate: rating.rate,    
        count: rating.count
      },
        "image":base64Images}
